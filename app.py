...
# =========================================================
# 3. STREAMLIT HELP DIALOG
# =========================================================
@st.dialog("📖 How to Read the Veracity Dashboard", width="large")
def show_help_guide():
    st.markdown("""
    ### Welcome to Veracity
    This dashboard transforms raw demographic and health data into actionable field strategy for Sun Pharma. Here is your guide to understanding the insights.

    ---

    #### 1. What the Data Columns Mean
    *   **Population Total & Pct Urban:** Represents the absolute market scale and retail density. High values mean higher addressable volume for acute therapies.
    *   **Pct Diabetes / Hypertension:** Key drivers for the **Chronic MAI**. High values suggest a need for specialist Medical Representatives (MRs) focusing on cardiology and diabetology.
    *   **Pct ARI Children:** Key driver for the **Acute MAI**. High values suggest a need for high-volume trade reps pushing anti-infectives and pediatrics.
    *   **Doctors/Beds per 1000:** Provider capacity. *Note:* Veracity scores this dynamically. Extremely low capacity is penalized, but *moderate* capacity coupled with high demand scores as a massive "Whitespace Opportunity".
    *   **Per Capita Income & PMJAY:** Represents out-of-pocket spending power and institutional insurance coverage.
    *   **Growth Trends (Urbanization, Income, NCD Risk):** These forward-looking indicators are projected forward to calculate the **Emerging Opportunity Index** and the **2031 Future MAI**.

    #### 2. How to Read the Matrices (Charts)
    *   **Therapy Skew vs Attractiveness:** 
        *   *X-Axis (Size):* How big is the market today?
        *   *Y-Axis (Therapy Bias):* Dots above the '0' line lean toward Chronic diseases. Dots below the line lean Acute. Use this to decide *which* drug portfolio to launch in a specific district.
    *   **Opportunity Gap Analysis:** 
        *   Look for dots in the **Top-Right**. These are "Hidden Gems." They possess high patient demand but have shockingly low competitor or doctor density. They are prime targets for immediate expansion.

    #### 3. How to Use the Executive Deep-Dive
    *   Don't trust the score blindly. Use Tab 3 to select any district.
    *   Veracity will reveal the exact **Positive Drivers (Strengths)** pushing the score up, and the **Negative Drivers (Weaknesses)** pulling it down, compared to the national average.
    *   Use the **Recommended Strategy** text to immediately guide territory planning (e.g., "Expand Immediately", "Pilot Market", "Monitor").
    """)
...
# Added Help Button
if st.sidebar.button("📖 How to Read This Dashboard", use_container_width=True):
    show_help_guide()
